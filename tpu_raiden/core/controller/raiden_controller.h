...
class RaidenController {
 public:
  ...
  // staging_host_buffers: host DRAM staging (bridge) buffers, required for
  // 2-stage remote transfers (remote H2D read/write, remote D2H write);
  // always the MIDDLE hop of the data flow (hence its position between
  // dst_buffers and copy_sizes). Each staging buffer must be a plain host
  // buffer: memory_type MEMORY_TYPE_DRAM (or UNSPECIFIED) and index = the
  // staging host block id; no remote endpoints (for H2dWrite the peer address
  // rides on dst_buffers). Unused for local transfers.
