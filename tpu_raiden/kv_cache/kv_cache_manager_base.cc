...
absl::StatusOr<raiden::PjRtCopyFuture> KVCacheManagerBase::H2dWrite(
  ...
  const uint64_t uuid =
      (1ULL << 63) |
      (static_cast<uint64_t>(node_id() & 0x3FFF) << 48) |
      ((static_cast<uint64_t>(reinterpret_cast<uintptr_t>(this) >> 4) &
        0xFFFF)
       << 32) |
  ...
  if (!push_pool_) {
    ASSIGN_OR_RETURN(auto h2h_res,
                     H2hWrite(std::string(peer), src_block_ids,
                              staging_block_ids, uuid, /*layer_idx=*/-1,
                              device_block_ids));
...
absl::StatusOr<raiden::PjRtCopyFuture> KVCacheManagerBase::H2dRead(
  ...
  if (num_chunks == 1 || !pull_pool_) {
    ASSIGN_OR_RETURN(auto h2h_fut,
                     H2hReadExplicit(std::string(peer), src_block_ids,
                                     staging_block_ids,
                                     /*explicit_dst_ptrs=*/{}));
  ...
  for (size_t i = 0; i < num_chunks; ++i) {
    ...
    pull_pool_->Schedule([this, state, peer_str, src_block_id,
                          staging_block_id, staging_offset, dst_device_offset,
                          size]() {
      if (state->HasFailed()) {
        state->MarkChunkComplete();
        return;
