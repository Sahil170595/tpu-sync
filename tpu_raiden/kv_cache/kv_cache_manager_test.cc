...
TEST(KVCacheManagerTest, RemoteTwoStageApisRequireExplicitStaging) {
  ...
  EXPECT_THAT(h2d_read.status().message(),
              testing::HasSubstr("same length"));
  ...
  EXPECT_THAT(d2h_write.status().message(),
              testing::HasSubstr("same length"));

  auto h2d_write = manager.H2dWrite("localhost:1", /*src_host=*/{0},
                                    /*dst_host(staging)=*/{}, /*dst_device=*/{0},
                                    /*copy_sizes=*/{1});
  EXPECT_EQ(h2d_write.status().code(), absl::StatusCode::kInvalidArgument);
  EXPECT_THAT(h2d_write.status().message(),
              testing::HasSubstr("same length"));
}
