...
TEST(PhysicalLiveRangeToLogicalTest, MapsWithinRunsAndRejectsPaddingCross) {
  ...
  absl::StatusOr<std::pair<int64_t, int64_t>> range = PhysicalLiveRangeToLogical(
      *segments, /*physical_offset=*/105, /*size=*/10);
  ...
  EXPECT_THAT(
      PhysicalLiveRangeToLogical(*segments, /*physical_offset=*/15, /*size=*/10)
          .status(),
      StatusIs(absl::StatusCode::kInvalidArgument, HasSubstr("crosses padding")));
}
