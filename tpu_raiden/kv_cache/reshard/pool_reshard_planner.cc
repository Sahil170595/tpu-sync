...
std::string GeometrySignature(const tpu_raiden::rpc::PoolSpecProto& pool) {
  std::string sig = absl::StrCat(pool.tag(), "\x1f", pool.storage_index(),
                                 "\x1f", pool.base_offset_bytes(), "\x1f",
                                 pool.block_stride_bytes(), "\x1f",
                                 pool.num_blocks(), "\x1f", pool.dtype_tag());
  for (const auto& region : pool.regions()) {
    absl::StrAppend(&sig, "\x1e", region.name(), "\x1f",
                    region.offset_bytes(), "\x1f", region.stride_bytes(),
                    "\x1f", region.unit_bytes(), "\x1f", region.num_units(),
                    "\x1f", region.units_per_stride());
  }
  return sig;
}
...
absl::StatusOr<std::map<RaidenId, const tpu_raiden::rpc::
                                      RegisterWorkUnitRequest*,
...
absl::StatusOr<PoolReshardPlan> BuildPoolReshardPlan(
  ...
  if (dst_ids.empty()) {
    return absl::InvalidArgumentError(
        "dst_device_block_ids must not be empty");
  }
  ...
  auto src_by_unit_or = MetadataByUnit(request.src_metadata,
                                       request.src_units);
  if (!src_by_unit_or.ok()) return src_by_unit_or.status();
  auto& src_by_unit = *src_by_unit_or;
  auto dst_by_unit_or = MetadataByUnit(request.dst_metadata,
                                       request.dst_units);
  ...
  for (const auto* meta : all_metadata) {
    ...
    if (meta->layout_fingerprint().empty()) {
      return absl::InvalidArgumentError(absl::StrCat(
          "Missing layout fingerprint for ", PythonRepr(unit)));
    }
    ...
    if (meta->shards_size() == 0) {
      return absl::InvalidArgumentError(absl::StrCat(
          "Missing data-plane endpoint for ", PythonRepr(unit)));
    }
      ...
      *src_by_unit.at(request.src_units[0]);
  {
    ...
    for (size_t i = 1; i < request.src_units.size(); ++i) {
      ...
      if (geometry != reference_geometry) {
        return absl::InvalidArgumentError(absl::StrCat(
            "Source pool geometry differs across ranks at ",
            PythonRepr(src_unit)));
      }
    }
  }
  ...
  for (const RaidenId& unit : request.src_units) {
    admitted_parallelism.insert(
        src_by_unit.at(unit)->transfer_parallelism());
  }
  if (admitted_parallelism.size() != 1 ||
      admitted_parallelism.count(0) > 0) {
    return absl::InvalidArgumentError(
  ...
  for (size_t group_idx = 0; group_idx < requested_tags.size();
       ++group_idx) {
    ...
    const std::vector<int64_t>& dst_ids_g = tag_dst_ids[group_idx];
    {
      ...
      if (unique_ids.size() != dst_ids_g.size()) {
        return absl::InvalidArgumentError(absl::StrCat(
            "dst_device_block_ids for tag ", PyStrRepr(plan_tag),
    ...
    for (int32_t pool_idx : precheck.selected) {
      ...
      if (src_live <= 0 || dst_live <= 0) {
        return absl::InvalidArgumentError(absl::StrCat(
            "Pool ", pool_idx, " has no declared live bytes"));
      }
  ...
  for (size_t group_idx = 0; group_idx < requested_tags.size();
    ...
    if (declared.empty()) {
      return absl::InvalidArgumentError(absl::StrCat(
          "No byte-span declarations are registered for tag ", plan_tag,
    ...
    for (int32_t pool_idx : precheck.selected) {
      ...
      for (const auto& [unit, entry] : declared) {
        ...
        for (int64_t block_id : entry->block_ids) {
          if (block_id >= limit) {
            return absl::InvalidArgumentError(absl::StrCat(
                "Source block id is out of range for pool ", pool_idx,
                " at ", PythonRepr(unit)));
          }
        }
      }
    ...
    for (const auto& [unit, entry] : declared) {
      ...
      for (const PoolByteSpan& span : entry->spans) {
        if (span.dst_block_index >=
            static_cast<int64_t>(dst_ids_g.size())) {
          return absl::InvalidArgumentError(absl::StrCat(
              "Byte span destination index ", span.dst_block_index,
        ...
        if (src_end > src_live) {
          return absl::InvalidArgumentError(absl::StrCat(
              "Byte span exceeds its source block live bytes: end=",
              src_end, ", live=", src_live, " (declared by ",
              PythonRepr(unit), ")"));
        }
        ...
        if (dst_end > dst_live) {
          return absl::InvalidArgumentError(absl::StrCat(
              ...
              dst_end, ", live=", dst_live, " (declared by ",
              PythonRepr(unit), ")"));
        }
        ...
        for (int32_t repeat = 0; repeat < span.count; ++repeat) {
          ...
          coverage[span.dst_block_index].emplace_back(
    ...
    std::stable_sort(ordered_spans.begin(), ordered_spans.end(),
                     [](const OrderedSpan& a, const OrderedSpan& b) {
                       return std::tie(a.span->dst_block_index,
                                       a.span->dst_offset_bytes) <
                              std::tie(b.span->dst_block_index,
                                       b.span->dst_offset_bytes);
                     });
    std::map<RaidenId,
             std::set<std::tuple<std::string, int64_t, int64_t>>,
             RequestBlockRegistry::RaidenIdLess>
    ...
    for (const OrderedSpan& ordered : ordered_spans) {
      ...
      for (int32_t repeat = 0; repeat < span.count; ++repeat) {
        ...
        const int64_t dst_offset =
            span.dst_offset_bytes + repeat * span.dst_stride_bytes;
        auto translated = TranslateLiveCopy(
            precheck.src_segments, precheck.dst_segments, src_offset,
            dst_offset, span.size_bytes);
