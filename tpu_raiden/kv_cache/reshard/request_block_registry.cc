...
absl::Status RequestBlockRegistry::Register(
    ...
    for (const PoolSpanRegistration& entry : pool_spans) {
      ...
      for (const PoolByteSpan& span : entry.spans) {
        ...
        if (span.src_block_ordinal < 0 ||
            span.src_block_ordinal >=
                static_cast<int64_t>(entry_ids.size())) {
          return absl::InvalidArgumentError(absl::StrCat(
              "byte span ordinal ", span.src_block_ordinal,
        ...
        if (src_end > live) {
          return absl::InvalidArgumentError(absl::StrCat(
              "byte span exceeds its source block live bytes (", tag,
              "): end=", src_end, ", live=", live));
        }
      ...
      if (entry.declared_bytes != span_bytes) {
        return absl::InvalidArgumentError(absl::StrCat(
            ...
            entry.declared_bytes, " disagrees with the span sum ",
            span_bytes));
      }
  ...
  if (cancelled_.find(lifecycle_key) != cancelled_.end()) {
    return absl::InvalidArgumentError(
        absl::StrCat("Request block registration was cancelled for req_id=",
                     req_id, ", uuid=", uuid));
  }
  ...
  if (existing != request_blocks_.end()) {
    ...
    if (current.uuid != uuid || current.block_ids != normalized ||
      ...
      return absl::InvalidArgumentError(
          absl::StrCat("Conflicting request block registration for req_id=",
                       req_id, ", unit=", PythonRepr(unit)));
    }
...
absl::StatusOr<int> RequestBlockRegistry::Complete(const std::string& req_id,
  ...
  auto [completion_it, inserted] =
      completed_units_.try_emplace(lifecycle_key);
...
absl::StatusOr<
    std::map<RaidenId, RequestBlockRegistration,
...
bool RequestBlockRegistry::AbandonClaim(const std::string& req_id,
                                        int64_t uuid,
                                        const void* claim_owner) {
  const LifecycleKey lifecycle_key{req_id, uuid};
  absl::MutexLock lock(mu_);
