...
std::string EncodeStartTransfer(const PoolReshardPlan& plan,
  ...
  *start_req->add_dst_units() = RaidenIdToProto(plan.dst_unit);
  ...
  if (target == plan.dst_unit) {
    ...
    if (schedule_it != plan.schedules.end() &&
        !schedule_it->second.empty()) {
      tpu_raiden::rpc::ShardPushScheduleProto schedule_proto;
...
ReshardCoordinator::QueryRemoteMetadata(const std::string& address) {
  ...
  if (!resp.success()) {
    return absl::InternalError(absl::StrCat(
        "Failed to query remote metadata: ", resp.message()));
  }
...
absl::Status ReshardCoordinator::StartPoolReshard(
...
absl::Status ReshardCoordinator::ExecutePoolReshard(
  ...
  std::vector<absl::Status> sender_status(plan.src_units.size());
  {
    ...
    for (size_t i = 0; i < plan.src_units.size(); ++i) {
      ...
      senders.emplace_back([this, &plan, &sender_status, i, unit]() {
        ...
        sender_status[i] =
            SendWorkerRpc(transport_, addr_it->second, payload);
      });
    }
  ...
  for (size_t i = 0; i < sorted_tags.size(); ++i) {
    absl::StrAppend(&tags_json, i ? ", " : "", "\"",
                    JsonEscape(sorted_tags[i]), "\"");
  }
  std::string skipped_json;
  {
    bool first = true;
    for (const auto& [tag, count] : plan.skipped_pool_counts) {
      absl::StrAppend(&skipped_json, first ? "" : ", ", "\"",
                      JsonEscape(tag), "\": ", count);
      first = false;
    }
  }
