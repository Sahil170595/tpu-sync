...
class ReshardStackTest : public ::testing::Test {
 protected:
  ...
  rpcpb::ControllerResponse Coordinate(const std::string& req_id,
                                       int64_t uuid, int num_src,
...
TEST_F(ReshardStackTest, FullPoolReshardFlowEmitsArmThenDispatch) {
  ...
  for (int i = 1; i <= 2; ++i) {
    ...
    EXPECT_EQ(dispatch.start_transfer_request().shard_push_schedules_size(),
              1);
    const auto& schedule =
...
TEST_F(ReshardStackTest, CancelTombstoneBlocksLateRegistration) {
  ...
  cancel_req.mutable_cancel_request_blocks_if_unclaimed_request()
      ->set_req_id("req-5");
  cancel_req.mutable_cancel_request_blocks_if_unclaimed_request()->set_uuid(
      46);
...
TEST_F(ReshardStackTest, TtlPurgesExpiredRegistrations) {
  ...
  ASSERT_FALSE(resp.success());
  EXPECT_THAT(resp.message(),
              HasSubstr("Missing producer block registration"));
}
