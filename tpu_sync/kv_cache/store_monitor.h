...
class StoreMonitor {
 public:
  ...
  // sweep until the next period or nudge.
  ...
  // Runs the sweep now instead of at its next period. Called from the
  // store's allocation path when free blocks dip below the sweep's low
 ...
 private:
  ...
  absl::Mutex mu_;
  // Signaled on Stop() and NudgeSweep(); the loop waits on it with a
